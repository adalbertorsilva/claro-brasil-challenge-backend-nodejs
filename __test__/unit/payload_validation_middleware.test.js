...
describe('PAYLOAD VALIDATION MIDDLEWARE UNIT TEST', () => {
  describe('CREATE DEVICE PAYLOAD', () => {
  ...
  describe('UPDATE DEVICE NAME PAYLOAD', () => {
    describe('when a payload is sent without an device_name field', () => {
      it('should throw a MissingFieldError', async () => {
        const payload = {}

        expect(() => { payloadValidationMiddleware.validateDeviceName(payload) }).toThrow(MissingFieldError)
      })
    })
  })
})
